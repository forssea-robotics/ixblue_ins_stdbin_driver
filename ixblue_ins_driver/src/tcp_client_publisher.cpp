      ...
      resolver(service)
{
    try {
        ...
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("tcp_publisher"), "Error while connecting to " << ip << ":" << port << " : " << e.what());
    }
}
...
void TCPClientPublisher::sendNextData(const ixblue_stdbin_decoder::Data::BinaryNav& binaryNav, uint32_t time_100us)
{
    ...
    try {
        RCLCPP_DEBUG(rclcpp::get_logger("tcp_publisher"), "Serializing data");
