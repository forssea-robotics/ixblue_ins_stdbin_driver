...
{
    node_->declare_parameter<std::string>("frame_id", std::string("imu_link_ned"));
    node_->get_parameter("frame_id", frame_id);
    node_->declare_parameter<std::string>("time_source", std::string("ins"));
    node_->get_parameter("time_source", time_source);
    node_->declare_parameter<std::string>("time_origin", std::string("unix"));
    node_->get_parameter("time_origin", time_origin);
    node_->declare_parameter<bool>("use_compensated_acceleration", false);
    node_->get_parameter("use_compensated_acceleration", use_compensated_acceleration);
    ...
    {
        RCLCPP_WARN(node_->get_logger(), "This timestamp source is not available. You can use ins or ros. By "
    ...
    {
        RCLCPP_WARN(node_->get_logger(), "This timestamp origin is not available. You can use unix or "
    ...
    RCLCPP_INFO(node_->get_logger(), "Timestamp register in the header will come from : %s", time_source.c_str());
    RCLCPP_INFO(node_->get_logger(), "Timestamp register in the header will be in the base time of : %s",
             time_origin.c_str());
    RCLCPP_INFO(node_->get_logger(), "Use compensated acceleration : %s",
    ...
    // Publishers
    stdImuPublisher = node_->create_publisher<sensor_msgs::msg::Imu>("standard/imu", 1);
    stdNavSatFixPublisher = node_->create_publisher<sensor_msgs::msg::NavSatFix>("standard/navsatfix", 1);
    stdTimeReferencePublisher =node_->create_publisher<sensor_msgs::msg::TimeReference>("standard/timereference", 1);
    stdInsPublisher = node_->create_publisher<ixblue_ins_msgs::msg::Ins>("ix/ins", 1);
}
    ...
    const ixblue_stdbin_decoder::Data::NavHeader& headerData)
{
    ...
    getHeader(headerData, navData, headerMsg);
    ...
    ixblue_ins_msgs::msg::Ins iXinsMsg;

    if(!useInsAsTimeReference)
    {
        sensor_msgs::msg::TimeReference timeReferenceMsg;

        if(toTimeReference(headerData, timeReferenceMsg))
        {
    ...
    if(toImuMsg(navData, use_compensated_acceleration, imuMsg))
    {
        imuMsg.header = headerMsg;
        stdImuPublisher->publish(imuMsg);
        diagPub.stdImuTick(imuMsg.header.stamp);
    }
    if(toNavSatFixMsg(navData, navsatfixMsg))
    {
                             ...
                             std_msgs::msg::Header & res)
{
    ...
    if(useInsAsTimeReference)
    {
        ...
        auto sec = (uint32_t)((headerData.navigationDataValidityTime_100us) / 10000);
        auto nsec =
        ...
        if(useUnixAsTimeOrigin)
        {
            ...
            auto survey_day_p = boost::posix_time::ptime(survey_day);
            auto unix_origin_p = boost::posix_time::ptime(unix_origin);
                            ...
                            sensor_msgs::msg::Imu & res)
{
    ...
    // --- Orientation SD
    if(!navData.attitudeQuaternionDeviation.is_initialized())
    {
    ...
    // --- Angular Velocity SD
    if(!navData.rotationRateVesselFrameDeviation.is_initialized())
    {
        // We don't have the velocity covariance in data, so according to the ROS standard
        // described in msg file we set the whole matrix to 0.
    ...
    if(!navData.accelerationVesselFrameDeviation.is_initialized())
    {
        // We don't have the velocity covariance in data, so according to the ROS standard
        // described in msg file we set the whole matrix to 0.
...
bool ROSPublisher::toNavSatFixMsg(const ixblue_stdbin_decoder::Data::BinaryNav & navData,
                                  sensor_msgs::msg::NavSatFix & res)
{

    // --- Check if there are enough data to send the message
    if(!navData.position.is_initialized())
    {
        return false;
    }
...
bool ROSPublisher::toTimeReference(const ixblue_stdbin_decoder::Data::NavHeader & headerData,
                                   sensor_msgs::msg::TimeReference & res)
{

    // --- System time
    res.header.stamp = node_->get_clock()->now();
...
bool ROSPublisher::toiXInsMsg(const ixblue_stdbin_decoder::Data::BinaryNav & navData,
                              ixblue_ins_msgs::msg::Ins & res)
{

    // --- Check if there are enough data to send the message
    if(!navData.position.is_initialized() ||
      !navData.attitudeHeading.is_initialized() ||
      !navData.speedVesselFrame.is_initialized() ||
      !navData.insUserStatus.is_initialized())
    {
        return false;
    }
    ...
    // --- Attitude SD
    if(!navData.attitudeHeadingDeviation.is_initialized())
    {
        // We don't have the velocity covariance in data, so according to the ROS standard
        // described in msg file we set the whole matrix to 0.
    ...
    // --- Speed ENU SD
    if(!navData.speedGeographicFrameDeviation.is_initialized())
    {
        // We don't have the velocity covariance in data, so according to the ROS standard
        // described in msg file we set the whole matrix to 0.
